...
export function hasFileStorage(env) {
  return Boolean(env?.FILES && typeof env.FILES.put === 'function');
}

export function requireFileStorage(env) {
  if (!hasFileStorage(env)) {
    throw new Error(
      'Armazenamento de arquivos (R2) não configurado. Habilite o binding FILES no wrangler.toml.'
    );
  }
}
...
export function hasTaskEveryTripDay(trip, tasks = []) {
  if (!trip || !trip.start_date || !trip.end_date || !Array.isArray(tasks)) {
    return false;
  }

  const requiredDays = daysBetween(trip.start_date, trip.end_date);
  const uniqueTaskDates = new Set(
    tasks
      .map((task) => String(task.task_date || '').trim())
      .filter(Boolean)
  );

  return uniqueTaskDates.size >= requiredDays;
}
