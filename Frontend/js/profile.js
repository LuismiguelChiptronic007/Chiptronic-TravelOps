...
function renderHistory(trips) {
  ...
  if (!trips.length) {
    body.innerHTML = `<tr><td colspan="4" class="empty-state">Nenhuma viagem ainda</td></tr>`;
    return;
  }
...
async function load() {
  const shellUser = await mountShell({ active: '' });
  if (!shellUser) return;
  ...
  try {
    ...
    document.getElementById('position_title').value = u.position_title;
...
document.getElementById('profile-form')?.addEventListener('submit', async (e) => {
  ...
  try {
    const res = await api.updateProfile({
      ...
      position_title: document.getElementById('position_title').value.trim(),
