...
## 0) Configurar JWT_SECRET (OBRIGATÓRIO — local e remoto)

O sistema usa tokens JWT para autenticação. Sem `JWT_SECRET` configurado, login e cadastro retornam **erro 500** ("JWT_SECRET não configurado").

### Desenvolvimento local
Copie o arquivo de exemplo e ajuste o valor se desejar:
```powershell
Copy-Item .dev.vars.example .dev.vars
```
O `.dev.vars` é usado automaticamente pelo `wrangler dev`. **Nunca** comite este arquivo — ele já está no `.gitignore`.

### Produção (Cloudflare Workers)
Publique a chave como secret do Worker (nunca coloque em `wrangler.toml`):
```powershell
wrangler secret put JWT_SECRET
```
Quando solicitado, cole uma string aleatória forte (mínimo 32 caracteres, ex.: saída de `openssl rand -hex 32`).
...
## Problemas comuns
- Erro `500 JWT_SECRET não configurado` → passo 0 não foi feito (local: `.dev.vars`; produção: `wrangler secret put JWT_SECRET`).
- Erro: `account_id` não encontrado → rode `wrangler whoami` e adicione `account_id` em `wrangler.toml`.
- Migrations não aplicam / erro `duplicate column name` → verifique `database_id` em `wrangler.toml` corresponde ao DB no painel Cloudflare; migrations são idempotentes, mas DBs antigos podem ter estado inconsistente.
- UI carrega mas API retorna 404/401 → confirme token JWT e CORS; se usar `API_BASE` relativo, o Worker servirá `/api` corretamente.
- Erro de conexão no frontend (login/cadastro) → verifique conexão com a internet; a mensagem antiga mencionando `http://127.0.0.1:8787/` foi removida.
